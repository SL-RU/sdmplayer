...
uint8_t SSD1306_Init(void) {
	...
	if (HAL_I2C_IsDeviceReady(&SSD1306_I2C, SSD1306_I2C_ADDR, 5, 1000) != HAL_OK) {
		/* Return false */
		elog("SSD1306 I2C device not found");
		return SYS_ERROR;
	}
...
void SSD1306_UpdateScreen(void) {
	...
	HAL_I2C_Master_Transmit_DMA(&SSD1306_I2C, SSD1306_I2C_ADDR, SSD1306_Buffer_all, SSD1306_WIDTH * SSD1306_HEIGHT / 8 + 1);
...
void ssd1306_I2C_WriteMulti_DMA(uint8_t address, uint8_t reg, uint8_t* data, uint16_t count) {	
	HAL_I2C_Master_Transmit(&SSD1306_I2C, address, &reg, 1, 100);
	HAL_I2C_Master_Transmit_DMA(&SSD1306_I2C, address, data, count);
}
